...
def test_entries_missing_usd_quote_are_skipped(mock_fg, mock_global, mock_listings):
    ...
    mock_listings.return_value = {
        "data": [
            {"symbol": "GOOD", "quote": _quote_list(3.0)},
    ...
    body = json.loads(resp["body"])
    assert body["top_gainers"] == [{"symbol": "GOOD", "change_24h": 3.0}]
    assert body["top_losers"] == [{"symbol": "GOOD", "change_24h": 3.0}]
