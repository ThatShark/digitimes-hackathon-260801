...
def test_sample_options_are_five_point_likert():
    _, questions = sample_questionnaire()
    for q in questions:
        assert [o["id"] for o in q["options"]] == ["1", "2", "3", "4", "5"]
...
def test_all_neutral_answers_score_50_on_every_axis():
    answers = [{"question_id": qid, "option_id": "3"} for qid in QUESTIONS_BY_ID]
...
def test_extreme_low_pole_answers_score_0():
    answers = [
        {"question_id": qid, "option_id": ("5" if q["pole"] == "low" else "1")}
...
def test_extreme_high_pole_answers_score_100():
    answers = [
        {"question_id": qid, "option_id": ("1" if QUESTIONS_BY_ID[qid]["pole"] == "low" else "5")}
...
def test_code_letters_low_pole_convention():
    answers = [
        {"question_id": qid, "option_id": ("5" if QUESTIONS_BY_ID[qid]["pole"] == "low" else "1")}
        for qid in QUESTIONS_BY_ID
    ]
    result = score_answers(answers)
...
def test_unknown_question_id_is_ignored():
    result = score_answers([{"question_id": "does-not-exist", "option_id": "5"}])
    assert result["axes"] == {"R": 50, "E": 50, "F": 50, "S": 50}
